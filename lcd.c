...
{	
	...
	const float AX = SR;
	const float BY = LR;
	const float ratio = BY/AX;
	for ( int x = Xcenter - SR; x <= Xcenter + SR; x++  )
	{
		float d1 = ratio*( sqrt( SR*SR - ( Xcenter - x )*( Xcenter - x ) ) );
		int D1 = round(d1);
		int y1 = Ycenter + D1;
		int y2 = Ycenter - D1;
		LCD_DrawDot( x, y1, usColor );
		LCD_DrawDot( x, y2, usColor );
	}

	for ( int y = Ycenter - LR; y <= Ycenter + LR; y++ )
	{
		float c1 = ( sqrt( LR*LR - ( Ycenter - y )*( Ycenter - y ) ) )/ratio;
		int C1 = round(c1);
		int x1 = Xcenter + C1;
		int x2 = Xcenter - C1;
		LCD_DrawDot( x1, y, usColor );
		LCD_DrawDot( x2, y, usColor );
	}
}
